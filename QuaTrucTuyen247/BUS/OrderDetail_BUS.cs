...
namespace BUS
{
    public class OrderDetail_BUS
    {
        ...
        {
            string sql = "select Product.ProductID, OrderDetail.UserID, Product.Photo, Product.ProductName, Product.Cost, OrderDetail.Amount, OrderDetail.Amount*Product.Cost as Total from Product inner join OrderDetail on Product.ProductID = OrderDetail.ProductID where OrderDetail.UserID ='" + ID + "'and OrderDetail.State='"+state+"'";
